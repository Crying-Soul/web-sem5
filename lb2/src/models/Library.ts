...
import { Book as BookInterface, BookCreateData, BookUpdateData, FilterOptions } from '../types/api.type.js';
...
export class Library {
    ...
    private async loadBooks(): Promise<void> {
        try {
            ...
            const parsed = JSON.parse(data || '[]') as BookInterface[];
    ...
    async addBook(bookData: BookCreateData): Promise<Book> {
    ...
    async borrowBook(id: string, borrowerName: string, days: number = 14): Promise<Book> {
        const book = this.books.get(id);
        if (!book) throw new Error('Book not found');
    ...
    filterBooks(options: FilterOptions): Book[] {
