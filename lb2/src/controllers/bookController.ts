...
import { BookFilters } from '../types.js';
...
export class BookController {
  ...
  static async listPage(req: Request, res: Response) {
    try {
      ...
      if (typeof q.isAvailable === 'string') {
        filters.isAvailable = q.isAvailable === 'true';
      }
      if (typeof q.isOverdue === 'string' && q.isOverdue === 'true') {
      ...
      res.render('books/list', {
        books,
        filters: {
          isAvailable: (q.isAvailable as string) ?? '',
          isOverdue: (q.isOverdue as string) ?? ''
        },
  ...
  static editForm(req: Request, res: Response) {
    try {
      ...
      res.status(500).render('error', {
        message: 'Ошибка при загрузке формы редактирования',
        title: 'Ошибка'
      });
    }
  }
  ...
  static async apiCreate(req: Request, res: Response) {
    try {
      ...
      if (!author?.trim()) {
        return res.status(400).json({ error: 'Автор книги обязателен' });
      }
      if (!publicationDate) {
        return res.status(400).json({ error: 'Дата публикации обязательна' });
      }

      const publicationDateObj = new Date(publicationDate);
      if (isNaN(publicationDateObj.getTime())) {
        return res.status(400).json({ error: 'Неверный формат даты публикации' });
      }
  ...
  static async apiUpdate(req: Request, res: Response) {
    try {
      ...
      if (publicationDate) {
        ...
        payload.publicationDate = publicationDateObj.toISOString();
      }

      if ((req as any).file) {
        payload.coverImage = (req as any).file.filename;
      }
  ...
  static async apiBorrow(req: Request, res: Response) {
    try {
      ...
      if (!borrowerName?.trim()) {
        return res.status(400).json({ error: 'Имя читателя обязательно' });
      }
      if (!dueDate) {
        return res.status(400).json({ error: 'Дата возврата обязательна' });
      }
      ...
      if (isNaN(dueDateObj.getTime()) || dueDateObj.getTime() < today.getTime()) {
        return res.status(400).json({ error: 'Неверная дата возврата' });
      }

      const result = await library.borrow(
        req.params.id,
        borrowerName.trim(),
      ...
      if (result === undefined) {
        return res.status(404).json({ error: 'Книга не найдена или недоступна' });
      }
      if (result === null) {
        return res.status(400).json({ error: 'Неверная дата возврата' });
      }
  ...
  static async apiReturn(req: Request, res: Response) {
    try {
      ...
      if (!result) {
        return res.status(404).json({ error: 'Книга не найдена или уже возвращена' });
      }
      return res.json(result);
    } catch (error) {
