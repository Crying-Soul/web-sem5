...
export function isValidBorrowPeriod(days) {
...
export function validateUpdateBookPayload(payload) {
    const errors = [];
    const value = {};
    ...
    if (payload.publicationDate !== undefined) {
        ...
        else if (!isValidISODateString(publicationDate))
            errors.push('Publication date is invalid');
        else {
    ...
    if (payload.isbn !== undefined) {
        const isbn = toStr(payload.isbn);
        if (!isbn)
            errors.push('ISBN cannot be empty');
        else if (!isValidISBN(isbn))
            errors.push('ISBN looks invalid');
        else
            value.isbn = isbn;
    }
...
export function validateBorrowPayload(payload) {
    const errors = [];
