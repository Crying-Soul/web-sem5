...
import { validateCreateBookPayload, validateUpdateBookPayload, validateBorrowPayload, createSuccessResponse, createErrorResponse, isAjaxRequest } from '../utils/utils.js';
...
export class BookController {
    ...
    static async updateBookForm(req, res, next) {
        try {
    ...
    static async getAllBooks(req, res, next) {
        try {
            ...
            if (searchQuery) {
                ...
                books = library.filterBooks({ status: status, showOverdue });
            }
    ...
    static async createBook(req, res, next) {
        try {
            ...
            if (req.file && 'filename' in req.file) {
                ...
                // Для JSON запросов, если coverImage передается как строка
                bookData.coverImage = body.coverImage;
            }
            const book = await library.addBook(bookData);
            if (isAjaxRequest(req)) {
                res.status(201).json(createSuccessResponse(book.toJSON(), 'Книга успешно добавлена'));
                return;
            }
    ...
    static async updateBook(req, res, next) {
        try {
            ...
            if (isAjaxRequest(req)) {
                res.status(200).json(createSuccessResponse(book.toJSON(), 'Книга успешно обновлена'));
                return;
            }
    ...
    static async borrowBook(req, res, next) {
        try {
            ...
            const book = await library.borrowBook(req.params.id ?? '', borrowData.borrowerName, borrowData.days);
            if (isAjaxRequest(req)) {
                res.status(200).json(createSuccessResponse(book.toJSON(), 'Книга успешно взята в аренду'));
                return;
            }
    ...
    static async returnBook(req, res, next) {
        try {
            ...
            if (isAjaxRequest(req)) {
                res.status(200).json(createSuccessResponse(book.toJSON(), 'Книга успешно возвращена'));
                return;
            }
